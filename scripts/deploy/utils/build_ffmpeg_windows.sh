#!/bin/bash
# to be run from the ffmpeg sources
# this script has to be run from MSYS launched from Visual Studio Command line as described here 
# https://trac.ffmpeg.org/wiki/CompilationGuide/MSVC and here https://www.ffmpeg.org/platform.html#Microsoft-Visual-C_002b_002b-or-Intel-C_002b_002b-Compiler-for-Windows
# should start MSYS with -use-full-path parameter like this: C:\msys64\msys2_shell.cmd -use-full-path

COMMON_OPTIONS="--toolchain=msvc --enable-version3 --disable-static --enable-shared --arch=x86_64 --logfile=logfile.txt"
